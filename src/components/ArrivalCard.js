...
export default function ArrivalCard({ sample, active, index }) {
  const currentLike = useSelector(selectCurrentLike(sample.id));
  const dispatch = useDispatch();
  const navigate = useNavigate();
  return (
    <div
      onClick={() => navigate(`/card/${sample.id}`)}
      className={`card ${index == active ? "card-active" : ""}`}
    >
      <div className="card-image-wrapper-arrival">
